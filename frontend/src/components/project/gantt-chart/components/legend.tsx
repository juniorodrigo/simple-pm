import { AlertTriangleIcon, ClockIcon, CheckIcon, CalendarIcon, TextIcon, Minimize2Icon } from "lucide-react";
import { memo } from "react";
import { FilterGroup, FilterState, LegendProps, SortOption } from "../types";
import { cn } from "@/lib/utils";

export const Legend = memo(
	({ showLegend, setShowLegend, filters, onFilterChange, sortOption, onSortChange, compactMode, onCompactModeChange }: LegendProps) => {
		...
		return (
			<div className="flex flex-wrap items-center gap-4 text-xs p-3 bg-muted/30 rounded-lg border">
				...
				<button
					...
					className={cn("flex items-center gap-2 px-2 py-1 rounded-md transition-colors", compactMode ? "bg-primary/10" : "hover:bg-muted/50")}
				>
					<Minimize2Icon className="h-3 w-3" />
					<span className={cn("text-muted-foreground", compactMode && "text-foreground")}>Modo compacto</span>
					{compactMode && <CheckIcon className="h-3 w-3" />}
				</button>
	...
	(prevProps, nextProps) => {
		return prevProps.showLegend === nextProps.showLegend && prevProps.filters === nextProps.filters && prevProps.sortOption === nextProps.sortOption && prevProps.compactMode === nextProps.compactMode;
	},
);
