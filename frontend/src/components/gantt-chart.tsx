...
export default function GanttChart({ activities }: GanttChartProps) {
  ...
  useEffect(() => {
    ...
    let latestDate = new Date(activities[0].dueDate)
        ...
        <div className="flex border-b sticky top-0 bg-background z-10">
          ...
          <div className="flex-1 flex">
            {dateRange.map((date, index) => (
              <div
                key={index}
                className={`w-10 flex-shrink-0 text-center text-xs py-2 ${
              ...
              <div className={`w-64 min-w-64 p-2 border-r ${getStatusColor(activity.status)}`}>
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <Avatar className="h-6 w-6">
                    <AvatarImage src="/placeholder-user.jpg" alt={activity.assignee} />
                    <AvatarFallback>{getInitials(activity.assignee)}</AvatarFallback>
                  </Avatar>
                </div>
              </div>
